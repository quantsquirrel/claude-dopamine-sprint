...
이미 `~/.claude/plugins/local/claude-dopamine-sprint`로 심링크되어 있다면 바로 사용 가능합니다.

수동 설치:
```bash
ln -sf ~/dev/tools/claude-dopamine-sprint ~/.claude/plugins/local/claude-dopamine-sprint
```
...
학습 상태는 `~/.claude/claude-dopamine-sprint/state.json`에 저장됩니다.
